...
const Profile: React.FC = () => {
  ...
  const renderPreview = (key: UploadKey) => {
    ...
    const baseImgClass = isProfile
      ? 'w-48 h-48 rounded-full object-cover border-2 border-border'
      : 'w-60 h-40 rounded-lg object-cover border-2 border-border shadow-sm'
    ...
    if (preview) {
      if (isProfile) {
        return (
          <div
            className="relative w-48 h-48 rounded-full border-2 border-border overflow-hidden cursor-pointer hover:opacity-80"
            onClick={handleUploadClick}
          >
            <img
              src={preview}
      ...
      const placeholderClass = isProfile
        ? 'relative w-48 h-48 rounded-full border-2 border-dashed border-input bg-muted flex flex-col items-center justify-center cursor-pointer hover:border-primary'
        : 'w-60 h-40 rounded-lg border-2 border-dashed border-input bg-muted flex flex-col items-center justify-center cursor-pointer hover:border-primary'
      return (
        <div className={placeholderClass} onClick={handleUploadClick}>
          <Upload className="h-6 w-6 text-muted-foreground mb-1" />
          <p className="text-xs text-muted-foreground font-semibold">Click to upload</p>
          <p className="text-xs text-center mt-2 text-muted-foreground">Please upload a valid image type (e.g., JPG, PNG)</p>
        </div>
      )
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <Card>
          <CardContent className="grid grid-cols-1 lg:grid-cols-3 gap-4 space-y-4 lg:space-y-0">
            {/* Profile Photo - Left 1/3 on large screens */}
            <div className="space-y-2 lg:col-span-1 flex flex-col justify-center items-start">
              <input
                id="profile-photo"
              ...
              <div className="relative items-start">
                ...
                <Label className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 bg-primary text-primary-foreground px-5 py-2 rounded-full text-xs font-bold whitespace-nowrap">
                  {form.watch('jobRole') || ''}
                </Label>
              </div>
            </div>

            {/* Important Details - Right 2/3 on large screens */}
            <div className="lg:col-span-2 lg:-ml-40 grid grid-cols-1 lg:pt-8 md:grid-cols-2 gap-2 space-y-4 md:space-y-0">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 space-y-4 md:space-y-0">
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="tfnAbn">TFN or ABN Number</Label>
